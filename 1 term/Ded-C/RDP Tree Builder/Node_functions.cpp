...
int Node_Add_Left (root_t* tree, tree_t* node, tree_t* child)
{
    ...
    if (node->left == NULL)
    {
        ...
        ISERR (Tree_Update_Sizes (tree, node, child->size), node);
    }
...
int Node_Delete (root_t* tree, tree_t* node)
{
    ...
    if (node->left)
    {
        ISERR (Node_Delete (tree, node->left), node->left);
    }

    if (node->right)
    {
        ISERR (Node_Delete (tree, node->right), node->right);
    }
