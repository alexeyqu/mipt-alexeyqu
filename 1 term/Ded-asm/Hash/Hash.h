...
#define HTML_TAB //"&nbsp &nbsp &nbsp &nbsp &nbsp"

#define _CHECK_( object )\
    if (Hashtable_Error_Catch(#object##_Is_OK (object), "", object) == HT_ERR) ASSERT (!"Here it was:");

#define KEY_CHECK_( key )\
    if (key == NULL)\
    {\
        OUT (The_Log, ""HTML_TAB" \tKey is not allocated. <br>\n"); \
        return ELEM_NULL_KEY;\
    }\
    if (strlen (key) >= Max_Len)\
    {\
        OUT (The_Log, ""HTML_TAB" \tToo big size of key. <br>\n"); \
...
#define INDEX_CHECK_( index )\
    ...
    {\
        OUT (The_Log, "Invalid index in list [%08x]. <br>\n", (int) list); \
        return LIST_WR_NDX_RANGE;\
    }

#define HTEC( func, object ) if (Hashtable_Error_Catch (func, "", object) == HT_ERR) ASSERT (!"Here it was:");

#define LIST_CHECK_( list, object ) HTEC (List_Is_OK (list), object);

#define ELEM_CHECK_( elem, object ) HTEC (Elem_Is_OK (elem), object);

#define HASHTABLE_CHECK_( hashtable, object ) HTEC (Hashtable_Is_OK (hashtable), object);
    ...
    #define ASSERT( cond )\
        if (!(cond))\
        {\
            fprintf (The_Log, "Assertion <%s> failed! <br>\n", #cond);\
            fprintf (The_Log, "Info: file %s, line %d <br><br>\n", __FILE__, __LINE__);\
            fprintf (stdout, "Assertion <%s> failed!\n ", #cond);\
            fprintf (stdout, "Info: file %s, line %d\n ", __FILE__, __LINE__);\
            fprintf (stdout, "See The_Log file pls..\n ");\
            exit (1);\
        }
#else
    #define OUT if (0)
...
extern FILE* The_Log;                       //!< The_Log file descriptor

typedef struct elem
{
    char key[Max_Len];  //!< Key of elem
    int amount;         //!< Some info about key
...
typedef struct list
{
    ...
    elem_t* head;        //!< Pointer to head element
} list_t;

typedef struct hashtable
{
    list_t* buckets;            //!< Pointer to set of lists
    unsigned int max_hashvalue; //!< Maximum hash value == Number of buckets
} hashtable_t;
 ...
 * @retval      ERR_HASHTABLE
******************************************************************************/
int Hashtable_Fill (hashtable_t* hashtable, char* data_input[], int (*hash) (const char* key, int* hash));
