...
bool COverlappedWindow::Create(HINSTANCE hInstance, const wchar_t className[])
{
	handle = ::CreateWindowEx(
		...
		L"Permanent Window Text",	 // Window text
...
{		
	...
	case WM_CTLCOLOREDIT:
		backgroundBrush = (HBRUSH)onColorEdit(wParam);
...
LRESULT COverlappedWindow::onColorEdit(WPARAM wParam)
{
	...
	if (isPreviewChecked)
	{
		...
		textColor = tmp.fontColor;
	}
	else
	{
		backgroundColor = current.backgroundColor;
		textColor = current.fontColor;
	}
...
void COverlappedWindow::onCreate()
{
	...
	::setFontSize(handleEdit, DEFAULTFONTSIZE);

	::SetWindowText(handleEdit, L"Permanent sample text");
}
