...
bool COverlappedWindow::Create(HINSTANCE hInstance, const wchar_t className[])
{
	...
	hInstanceLib = ::LoadLibrary(TEXT("WordsCountDll.dll"));

	if (hInstanceLib == NULL)
	{
		::MessageBox(NULL, L"ERROR LoadLibrary", L"ERROR", MB_ICONWARNING);
		return FALSE;
	}

	procAddr = (MYPROC) ::GetProcAddress(hInstanceLib, "wordsCount");

	if (procAddr == NULL)
	{
		::MessageBox(NULL, L"ERROR getProcAddr", L"ERROR", MB_ICONWARNING);
		return FALSE;
	}
...
void COverlappedWindow::countWordsInText()
{
	...
	_itow_s((procAddr)(textBuffer), nOfWords, 10);

	::MessageBox(NULL, nOfWords, L"WordsCountDYNAMIC", NULL);
}
