...
int main (int argc, char *argv[])
{
    ...
    for (i_sz = 0; i_sz < N_OF_SZS; i_sz++) 
    {
    	...
    	for (i_thr = 0; i_thr < N_OF_THR; i_thr++)
    	{
    		...
    		if (task_id == 0) gettimeofday(&t1, NULL);

			find_limits_mpi (&matrix, num_tasks, task_id, matrix_thr[i_thr], &max, &min);

	   		if (task_id == 0) gettimeofday(&t2, NULL);
...
int generate_matrix (int N, matrix_t *matrix)
{
	...
	{
		matrix->cells[i].real = generate_integer (-100, 100) + ((double)rand()/(double)RAND_MAX);
		matrix->cells[i].imag = generate_integer (-100, 100) + ((double)rand()/(double)RAND_MAX);
